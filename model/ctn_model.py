...
class Ctn(BaseModel):
    ...
    def name(self):
        return "contextual transformer netowrk"
    ...
    def modify_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_sty', type=float, default=1, help='weight for sty loss')

        return parser

    def __init__(self, opt):
        """Initial the ctn model"""
        ...
        if self.isTrain:
            ...
            self.optimizer_D = torch.optim.Adam(itertools.chain(filter(lambda p: p.requires_grad, self.net_D.parameters())),lr=opt.lr, betas=(opt.beta1, opt.beta2))
    ...
    def test(self):
        ...
        self.img_g = self.net_G(self.img_m, self.mask, 1 - self.mask)
